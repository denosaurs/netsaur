...
export const Weights = $root.Weights = (() => {
  ...
  Weights.toObject = function toObject(message, options) {
    ...
    if (message.quantizeMin != null && message.hasOwnProperty("quantizeMin")) {
      object.quantizeMin = options.json && !isFinite(message.quantizeMin)
        ? String(message.quantizeMin)
        : message.quantizeMin;
    }
    if (message.quantizeMax != null && message.hasOwnProperty("quantizeMax")) {
      object.quantizeMax = options.json && !isFinite(message.quantizeMax)
        ? String(message.quantizeMax)
        : message.quantizeMax;
    }
    return object;
  };
...
export const Model = $root.Model = (() => {
  ...
  Model.encode = function encode(message, writer) {
    ...
    if (
      message.kerasVersion != null && message.hasOwnProperty("kerasVersion")
    ) {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 6:
          ...
          message.modelWeights.push(
            $root.Weights.decode(reader, reader.uint32()),
          );
          break;
        default:
          reader.skipType(tag & 7);
  ...
  Model.verify = function verify(message) {
    ...
    if (
      message.modelWeights != null && message.hasOwnProperty("modelWeights")
    ) {
  ...
  Model.fromObject = function fromObject(object) {
    ...
    if (object.modelWeights) {
      ...
      for (let i = 0; i < object.modelWeights.length; ++i) {
        ...
        message.modelWeights[i] = $root.Weights.fromObject(
          object.modelWeights[i],
        );
      }
    }
    return message;
  ...
  Model.toObject = function toObject(message, options) {
    ...
    if (message.modelWeights && message.modelWeights.length) {
      ...
      for (let j = 0; j < message.modelWeights.length; ++j) {
        object.modelWeights[j] = $root.Weights.toObject(
          message.modelWeights[j],
          options,
        );
      }
    }
