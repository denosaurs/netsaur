...
const net = await new NeuralNetwork({
  silent: true,
  layers: [
    DenseLayer.fromJSON(JSON.parse(Deno.readTextFileSync("./examples/train_and_run/layer.json"))),
    DenseLayer.fromJSON(JSON.parse(Deno.readTextFileSync("./examples/train_and_run/output-layer.json")))
  ],
  cost: "crossentropy",
}).setupBackend(CPU);
