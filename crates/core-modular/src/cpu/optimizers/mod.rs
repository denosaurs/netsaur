            ...
            if let Some((params, grads, l)) = CPUOptimizer::get_params(layer) {
                match self {
                    CPUOptimizer::SGD(sgd) => {
                        sgd.update_grads(params, grads, rate, epoch, l)
                    }
                    CPUOptimizer::Adam(adam) => {
                        adam.update_grads(params, grads, idx, rate, l)
                    }
                    CPUOptimizer::Nadam(nadam) => {
                        nadam.update_grads(params, grads, idx, rate, l)
                    }
                    CPUOptimizer::RMSProp(rmsprop) => {
                        rmsprop.update_grads(params, grads, idx, rate, epoch, l)
                    }
                }
                idx += 1;
            }
